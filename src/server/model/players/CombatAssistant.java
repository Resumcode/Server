...
public class CombatAssistant{
	...
	**/
	...
	public void activatePrayer(int i) {
		...
		if (i == 25 && c.playerLevel[1] < 1) {
			c.getPA().sendFrame36(c.PRAYER_GLOW[i], 0);
			c.sendMessage("You need a Defence level of 20 to use Turmoil.");
			return;
		}
	...
	**/
	...
	public void activateSpecial(int weapon, int i){
		...
		switch(weapon) {
			...
			case 13739: // korasi sword
			c.startAnimation(3157);
			c.specDamage = 2.15;
			c.specAccuracy = 90.40;
			c.hitDelay = getHitDelay(c.getItems().getItemName(c.playerEquipment[c.playerWeapon]).toLowerCase());
			break;

			
			case 11700:
				c.startAnimation(4302);	//zgs	
				c.gfx0(1221);
				c.specDamage = 1.99;
				c.specAccuracy = 1.70;
				c.hitDelay = getHitDelay(c.getItems().getItemName(c.playerEquipment[c.playerWeapon]).toLowerCase());
