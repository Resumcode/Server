...
public class PestControl {
	...
	public final int WAIT_TIMER = 10; //Wait timer in boat
		...
		for (int j = 0; j < Server.playerHandler.players.length; j++) {
			if (Server.playerHandler.players[j] != null) {
				if (Server.playerHandler.players[j].inPcGame()) {
					...
					if (won && c.pcDamage > 4) {
						c.sendMessage("Congratz! You have been awarded @red@7@bla@ pest control points!");
						c.pcPoints += 7;
						...
						c.sendMessage("You didnt do shit.");
					} else {
						c.sendMessage("Defeated! You have not gained any points.");
					}
					c.pcDamage = 0;
